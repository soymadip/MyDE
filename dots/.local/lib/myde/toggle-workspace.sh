...
HISTORY_FILE="/tmp/myde_workspace_history"
...
if [[ $# -eq 1 ]]; then
  ...
  if [[ "$1" =~ ^[0-9]+$ ]]; then
    ...
    if [[ "$current_workspace" == "$1" ]]; then
      notify-send -u critical "Hyprland" "Alrady in workspace ${1}"
      exit 1
    else
