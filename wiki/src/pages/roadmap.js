...
export default function TasksPage() {
  ...
  const roadmap = customFields?.roadmap;
  const title = roadmap?.title;
  const description = roadmap?.description;
  const taskList = roadmap?.enable && roadmap?.taskList ? roadmap.taskList : [];

  // If roadmap is disabled, show a notice box instead
  if (!roadmap || !roadmap.enable) {
    return (
      <Layout
        title="Roadmap is Disabled"
        description="Roadmap is currently disabled"
      >
        <div className="tasks-container">
          <div className="tasks-content">
            <div className="tasks-disabled-notice">
              ...
              <h2 className="disabled-title">Roadmap is currently disabled</h2>
              <p className="disabled-help">
                To enable roadmap, set <code>roadmap.enable</code> to{" "}
                <code>true</code>
              </p>
            </div>
          </div>
    ...
    <Layout title={title} description={description}>
      <div className="tasks-container">
        <div className="tasks-header">
          ...
          {description && (
            <div className="tasks-description">{description}</div>
          )}
        </div>
        <div className="tasks-content">
