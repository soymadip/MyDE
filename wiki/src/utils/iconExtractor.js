 ...
 *
 * @param {React.ComponentType|string} reactIcon - Icon component, name from iconMap, or path like "ai/AiFillAlert"
 ...
 */
function extractSvg(reactIcon, outputPath = null, options = {}) {
  // Default output path
  if (!outputPath) {
    outputPath = path.join(path.resolve(__dirname, "../.."), "static", "img");
  }
  ...
  if (typeof reactIcon === "string") {
    if (reactIcon.includes("/")) {
      // ("ai/AiFillAlert")
      IconComponent = requireDynamicIcon(reactIcon);
      iconIdentifier = reactIcon.split("/")[1];
    } else {
      // Icon from the iconMap
      IconComponent = iconMap[reactIcon.toLowerCase()]?.icon;
      iconIdentifier = reactIcon;
    }

    if (!IconComponent) {
      throw new Error(`Icon "${reactIcon}" could not be resolved`);
    }
