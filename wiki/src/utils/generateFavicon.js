...
async function generateFavicons(context, options = {}) {
  ...
  const profilePicUrl =
    options.imagePath || siteConfig.customFields.projectIcon;
  ...
  const imgDir = path.join(staticBaseDir, "img");
  const imgStaticPath = "/img";
  ...
  try {
    ...
    const iconsToGenerate = ["si/SiGoogledrive", "md/MdNewReleases"];

    for (const icon of iconsToGenerate) {
      extractSvg(icon, imgDir, iconColor);
    }
      ...
      shortcuts: [
        {
          ...
          icons: [
            {
              src: `${imgStaticPath}/icon-docs.svg`,
              sizes: "any",
              type: "image/svg+xml",
              purpose: "any",
            },
        ...
        },
        {
          ...
          icons: [
            {
              src: `${imgStaticPath}/icon-releases.svg`,
              sizes: "any",
              type: "image/svg+xml",
              purpose: "any",
            },
    ...
    if (Array.isArray(response.images) && response.images.length > 0) {
      for (const image of response.images) {
        ...
        try {
          ...
          console.error(
            `[ERROR] Failed to write image ${image.name}:`,
            err.message,
          );
        }
      }
...
if (require.main === module) {
  ...
  const siteConfig = require("@site/docusaurus.config.js").default;
