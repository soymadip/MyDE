...
2.  **Define the `help` subcommand:** Inside the `help` block, create a `case` for its own arguments. The first thing should always be the `help` option, which defines the `cmd_map` array for the `help_menu`.
